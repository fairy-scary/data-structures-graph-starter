...
function friendsOfRecursion(name, adjacencyList, visited, maxDistance, currentDistance) {
  if (currentDistance === maxDistance) return Array.from(visited);
  currentDistance++
  for (let friend of adjacencyList[name]) {
    if (visited.has(friend)) continue;
    visited.add(friend)
    console.log(visited)
    return friendsOfRecursion(name, adjacencyList, visited, maxDistance, currentDistance)
  }
}
...
function friendsOf(adjacencyList, name, distance) {
  ...
  if (adjacencyList[name] == [name]) return []; 
  let visited = new Set()
  let currentDistance = 0
  return friendsOfRecursion(name, adjacencyList, visited, distance, currentDistance)
}

console.log(friendsOf(list, "Aliza", 1))
